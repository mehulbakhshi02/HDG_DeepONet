...
::GenerateMLData(const Solution & sol, LocalHeap & lh, const Solution & new_sol, vector<double> &u_coordinates, vector<double> &u_sol) {
...
// Loop over faces
  ...
  for(int i=0;i<ne;i++)
  {
    ...
    for (int ll = 0; ll < COMP; ++ll)
    {
      ...
      for (int pp = 0; pp < ml_elidata.nip; ++pp)
      {
        for (int ll=0; ll < COMP; ++ll)
        {

          ml_elidata.w[ll+pp] = ed.w[ll+pp]; // Solution at integration points (Mehul)

        }

        
          ml_elidata.qp[pp*D] = ed.qp[pp*D];  // Integration point coordinates (Mehul)
          ml_elidata.qp[pp*D+1] = ed.qp[pp*D+1];  
      }
