...
class UnifyingFramework : public NumProc {
  ...
  // For the ML 
  vector<ML_ElementData<D, COMP>*> ml_eldata;
  // Face data for ML inputs
  vector<ML_FacetData<D, COMP>*> ml_fadata;
  // Data written out for ML training
  vector<ML_OutputData<D, COMP>*> ml_outputdata;
  // Reconstruction of results from ML
  vector<ML_DeployData<D, COMP>*> ml_deploydata;
  ...
  void SolveAdjointSystem(const Solution & sol, Solution & adj, vector<double> & error, int & its, vector<double> & adj_coordinates, vector<double> & adj_sol, LocalHeap & lh);
  ...
  void SaveTecplot(const string & filename, int max_order, const Solution & sol, const Space & sp, vector<double> &coordinates, vector<double> &solution, LocalHeap & lh) const;
  ...
  void SaveSolution(const string & filename, const Solution & sol, const Space & fspace, vector<double> &coordinates, vector<double> &solution, LocalHeap & lh) const;
...
//anisotropic refinement
  ...
  // ML functions
  #include "ML/generatemldata.h"
  #include "ML/writemldata.h"
  #include "ML/deployml.h"
  #include "ML/mlunittest.h"
class Init
{ 
  public: 
    Init ();
